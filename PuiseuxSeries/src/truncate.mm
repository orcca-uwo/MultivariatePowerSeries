# Truncate 
# to truncate a multivariate Puiseux series using the
# internal power series degree.
export
Truncate :: static := proc(_self :: PuiseuxSeriesObject,
                           deg :: integer := -2,
...
export
AbsoluteDegreeBound :: static := proc(_self :: PuiseuxSeriesObject,
                                      deg :: integer,
                                      $)
  local signs;
  ...
  local v := [seq(_self:-e[_self:-ord[i]] + add(a[j] * _self:-rays[j][i], 
                  j = 1 .. n_rays), i = 1 .. n_ord)];
  v := [op(v)];
  local inequalities := {
    seq(add(signs[i] * v[i], i = 1 .. n_rays) <= deg, 
        signs in Iterator:-CartesianProduct([-1, 1] $ n_rays)),
    seq(a[i] >= 0, i = 1 .. n_rays)};

  try
    local solution := Optimization:-LPSolve(add(a[i], i=1..n_rays), 
                                            inequalities, ':-maximize', 
                                            ':-assume' = ':-integer', 
                                            ':-depthlimit' = 1000);
    return solution[1];
