...
export GrevLexGreater::static := proc(_self::LaurentSeriesObject, 
                                      ray1::{list(rational), Vector}, 
                                      ray2::{list(rational), Vector}, $)
    local s1, s2, i;

    if nops(ray1) <> nops(ray2) then
    ...
    if s1 < s2 then
        ...
        for i from nops(ray1) to 1 by -1 do
...
export Positive :: static:= proc(_self :: LaurentSeriesObject, 
                                 ray::{list(rational), Vector}, $)
    local i;
    local zero := [seq(0, nops(ray))];

    return _self:-GrevLexGreater(_self, ray, zero);
end proc;

# Look for the grevlex smallest element in a list
export Smallest::static := proc(_self :: LaurentSeriesObject, 
...
local SmallestGrevlexMonomial::static := proc(_self :: LaurentSeriesObject, 
                                               mons::list(ratpoly), 
                                               v_order::list(name), 
                                               sw::boolean := true,
                                               $)
...
local LookForGreatestGrevlexLess::static := proc(_self::LaurentSeriesObject, 
                                                ray::list(rational), $)
    local i;
    ...
    for i from nops(ray) to 2 by -1 do
        newRay[i] := ceil(ray[i]);
    end do;
