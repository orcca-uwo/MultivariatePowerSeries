...
##- opt : (optional) equation of the form `returnleadingcoefficient` = `optval`, where `optval` is `automatic` or `true` or `false`
##
##DESCRIPTION
...
##	or issues an error if this is not possible. Let 'x' be the main variable of 'u'.
##- The command ~PuiseuxFactorize(u)~ proceeds as follows.
##-- Firstly, Maple checks that the constant term, 'p0', of 'u' is non-zero. Maple verifies 
...
##	 the smallest non-zero term of 'u'.
##	 If its "analytic expression" is not known, then Maple makes use of a bound 'b' to
##	 check if 'a0' is zero up to that bound or not. If by Truncating
##	 the 'a0' up to 'b' Maple finds that is non-zero, then it does not
##	 modify 'u'. If this is not the case, then an error is returned. It is always
##	 possible to rerun the command with a higher bound 'b' (see below). 
##-- Secondly, let 'pn' be the leading coefficient of 'u' (the Puiseux series that is the
##   coefficient of the highest power, 'n', of the 'x'). Maple verifies if its "analytic
...
##-- Thirdly, let 'pn1' be the coefficient of degree 'n-1' of 'u'. Then, the 
##	 command ~TschirnhausenTransformation(v, pn1)~ is applied and call the 
##	 output of this algorithm 'w'. The goal of this algorithm is to make the term of degree 'n-1'
##	 equal to zero. 
##-- Next, an appropriate change of variables to 'w' is applied in order to make it 
##	 a univariate polynomial over power series. This allow us to apply the 
##	 ~HenselFactorize~ algorithm. After that, Maple reverts the change or variables
...
##	this can be set using `SetPuiseuxBound`, and its 
##	default value is 10. 
...
##> f := UnivariatePolynomialOverPowerSeries([PuiseuxSeries(1), PuiseuxSeries(0), PuiseuxSeries(x, [x=x^(1/3)]), PuiseuxSeries(x, [x=x^(1/2)]), PuiseuxSeries(x/(1+x), [x=x^(1/2)])], z);
...
##> map(print, map(Truncate, F,10)):
##- Let's now define 4 Puiseux series and a univariate polynomials
...
##> h := UnivariatePolynomialOverPowerSeries([PuiseuxSeries(0), PuiseuxSeries(0),a0,a2,a3], 'x');
##- Now let's try to factorize 'h'.
##> F := PuiseuxFactorize(h); 
##<(verification="testerror") "invalid"
##- We get an error since Maple is not able to determine if 'a0' is non-zero
##	or not. Let's try again but now with a ~Puiseux_bound~ of 21.
...
##  **In:Univ. Iagel.** Acta Math **38** (2000), pp. 279–282.##-(lead=indent) Mohammadali Asadi, Alexander Brandt, Mahsa Kazemi, Marc Moreno Maza, and Erik Postma. 
